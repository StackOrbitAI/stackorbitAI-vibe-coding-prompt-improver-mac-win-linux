...
// Prepend our portable node binary path to process.env.PATH
...
setTimeout(() => {
  ...
  const electronDev = spawn(
    'npx tsup src/main/index.ts src/main/preload.ts --out-dir dist/main --format cjs --external electron --watch --onSuccess "electron ."',
    [],
    {
      shell: true,
      stdio: 'inherit',
